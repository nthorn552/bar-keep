...
export default (props: MenuDisplayProps) => {
  ...
  useEffect(() => {
    if (
      activeMenuRecipe &&
      (!fullRecipe || fullRecipe.id !== activeMenuRecipe.id)
    ) {
      if (!props.menu.includes(activeMenuRecipe)) {
        setActiveMenuItem(null);
      } else if (!fullRecipeLoading) {
        setFullRecipeLoading(true);
        barBackApi
          .get(`/recipes/${activeMenuRecipe.id}`)
          .then((res) => {
        ...
        console.log(
          "TODO: consider canceling request if selected recipe changes before fetch is complete?"
        );
      }
  ...
  switch (props.menuState) {
    case MenuState.READY:
        ...
        props.menu.map((menuRecipe) => {
          return (
            <MenuDisplayItem
              ...
              recipe={menuRecipe}
              isActive={
                activeMenuRecipe && menuRecipe.id === activeMenuRecipe.id
              }
              clickHandler={(recipe: Recipe) => {
                setActiveMenuItem(
                  activeMenuRecipe && recipe.id === activeMenuRecipe.id
                    ? null
                    : recipe
                );
              }}
      ...
      if (fullRecipeLoading) {
        ...
        fullRecipe &&
        !props.menu.find((recipe) => recipe.id === fullRecipe.id)
      ) {
        setFullRecipe(null);
      } else if (fullRecipe) {
    ...
    case MenuState.ERROR:
      menuDisplay = (
        <div className={classes.pageMessageContainer}>
          Uh oh, something went wrong and we couldn't create the menu...
        </div>
      );
      break;
  }
  ...
  return (
    <div className={classes.root}>
      <div className={classes.menuPage}>{menuDisplay}</div>
      <div className={classes.menuPage}>{recipeDisplay}</div>
    </div>
  );
};
